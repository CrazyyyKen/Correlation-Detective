...
def prepare_pearson_dataset(dataset_path: str) -> PreparedDataset:
    return prepare_dataset(dataset_path, l2norm)


def score_pearson_correlation(
    dataset_path: str,
    lhs: list[int] | tuple[int, ...],
    rhs: list[int] | tuple[int, ...],
) -> float:
    prepared = prepare_pearson_dataset(dataset_path)
    return score_pearson_from_prepared(prepared, lhs, rhs)


def score_pearson_from_prepared(
...
def search_pearson_correlation(
    dataset_path: str,
    max_p_left: int = 1,
    max_p_right: int = 1,
    ...
    results = (
        build_result(
            ...
            lhs,
            rhs,
            score_pearson_from_prepared(prepared, lhs, rhs),
        )
...
def main() -> None:
    parser = common_arg_parser(
        "Exhaustive Pearson correlation search using CorrelationDetective preprocessing.",
        two_sided=True,
    )
    args = parser.parse_args()
    results = search_pearson_correlation(
        args.dataset_path,
